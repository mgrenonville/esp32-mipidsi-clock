...
use slint::{
    platform::{
        software_renderer::{self, MinimalSoftwareWindow, RepaintBufferType, Rgb565Pixel},
...
fn main() {
    ...
    thread::scope(|scope| {
        ...
        executor.run(|spawner| {
            spawner
                .spawn(main_task(spawner, tx_render, rx_event, pool))
                .unwrap();
        });
    });
}
...
async fn main_task(
    ...
    let wall_clock = Rc::new(WallClockSim {});

    let _ = spawner.spawn(update_timer());

    // run the gui controller loop
    let mut controller = Controller::new(&main_window, hardware, wall_clock);
